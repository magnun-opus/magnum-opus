...
tRPC is ordinary HTTP, so a persona could always reach it by writing paths
out longhand. Three details made that miserable, and the adapter removes all
three: input encoding, the JSON-RPC envelope, and transformer wrappers.

```json
{
  "persona": "trpc_customer",
  "protocol": "trpc",
  "trpc": { "prefix": "/api/trpc", "transformer": "superjson" },

  "probes": {
    "accounts": { "procedure": "banking.overview", "type": "query",
                  "input": { "owner": "{{actor.sessionId}}" } }
  },
  "invariants": [
  ...
  "states": [
    { "name": "transfer",
      "on_enter": {
        "event_type": "trpc", "procedure": "banking.transfer",
        "type": "mutation", "intent": "transfer",
        "input": { "owner": "{{actor.sessionId}}", "amount": 5000 }
      },
      "transitions": [ { "to": "done", "condition": "on_success" } ] }
  ]
}
```

Note what is **absent**: no `?input=` to hand-encode, and the invariant reads
`accounts.accounts` rather than `accounts.result.data.json.accounts`.
Responses are unwrapped before extraction, logging and invariants, so the
envelope never reaches a persona author. A forgotten prefix would otherwise
resolve to nothing and report a pass on an invariant that checked air.

Queries carry input JSON-stringified in a query parameter; mutations send it
as the body; nested procedures are dotted (`banking.transfer`). The logged
action omits the query string, so findings group by procedure rather than by
each actor's distinct input — which also keeps fingerprints stable for
`diff`.

Batching is deliberately not used. Batched calls join procedures with commas
and return `207 Multi-Status`, which makes per-procedure timing and outcome
ambiguous — and that is most of what the event log is for.

```
npx magnum-opus init --domain banking-trpc
```
...
Available: `banking`, `banking-trpc`, `ecommerce`, `erp`, `food-delivery`,
`logistics`, `marketplace`, `saas` — 17 personas in total.
...
```
npx magnum-opus report                    # most recent run
npx magnum-opus report --run-id <id>
npx magnum-opus run --format json,sarif,junit,html
```

A single self-contained HTML file — no server, no external assets, no
listening port. It opens offline and attaches to a pull request.

The centrepiece is not the charts, it is the **trace timeline**. A summary
tells you a duplicate happened; the timeline lets you watch it happen:

```
[ok]    http_request   GET /products/2     HTTP 200 · 2ms
[ok]    http_request   POST /cart          HTTP 201 · 2ms
[warn]  http_request   POST /checkout      1200ms · attempt 1
[warn]  wait           wait_700ms          700ms
[ok]    late_response  POST /checkout      HTTP 201 · 2204ms · attempt 1
[warn]  http_request   POST /checkout      1200ms · attempt 2
[warn]  decision       terminal:abandon
[ok]    late_response  POST /checkout      HTTP 201 · 2204ms · attempt 2
[bad]   invariant      one order per intent
```

Order 2283281 and order 2283294, from one intent. Traces cited as evidence
open by default; a few clean ones are included for contrast, because a report
showing only failures gives no sense of what normal looks like.
