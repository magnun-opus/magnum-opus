...
Run options:
  ...
  --format <list>            json,sarif,junit       (default: json)
...
const FLAGS = new Set(['i-know-this-is-not-production', 'non-interactive']);
...
async function main() {
  ...
  try {
    switch (args.command) {
      case 'init':
        init({ dir: args.dir || 'magnum', domain: args.domain || null });
        break;
      case 'generate':
        if (!args.spec) {
