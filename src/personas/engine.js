...
async function runPersona(persona, { baseUrl, logger, registry, rng, actor, chaos = null }) {
  ...
  while (steps++ < MAX_STEPS) {
    ...
    if (state.on_enter) {
      ...
      if (spec.event_type === 'http_request') {
...
async function performRequest({
  ...
  const action = interpolate(spec.action, scope);
  const [method, path] = action.split(' ');
  const body = spec.body ? interpolate(spec.body, scope) : undefined;
  const headers = {
    ...(context.authHeaders || {}),
  ...
  if (settled.resolved) {
    ...
    applyExtract(spec.extract, r.body, context, rng);
    ...
    return logger.log({
      ...
      idempotencyKey,
      attempt,
      payload: { request: body ?? null, response: r.body, error: r.error, fault },
  ...
  handle.onLateSettle((result) => {
    logger.log({
      ...
      idempotencyKey,
      attempt,
      payload: { request: body ?? null, response: result.body, error: result.error },
      tags: ['late_response'],
